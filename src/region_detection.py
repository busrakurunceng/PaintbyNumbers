...
def _boundary_dominant_ratio(
    cleaned_region: np.ndarray,
    region_id: int,
    region_mask: np.ndarray,
) -> tuple:
    """Küçük bölgenin sınırında hangi komşunun kaç oranda yer aldığını verir.
    Returns:
        (dominant_neighbor_id, ratio 0-1) veya (None, 0) yeterli komşu yoksa.
    """
    dilated = cv2.dilate(
    ...
    dominant_id = np.argmax(counts)
    ratio = counts[dominant_id] / len(neighbor_ids)
    return int(dominant_id), float(ratio)
...
def remove_small_regions(
    ...
    portrait_enclosed_ratio: float = 0.88,
) -> tuple:
    ...
    Portrait modunda: detay bölgesinde, tek komşuyla neredeyse tam çevrili
    küçük bölgeler (yüzdeki patch) kontrasta bakılmaksızın birleştirilir.
    """
    ...
    for region_id in unique_regions:
        ...
        dominant_neighbor, boundary_ratio = _boundary_dominant_ratio(
            cleaned_region, region_id, region_mask
        )
        if dominant_neighbor is None:
            continue
        ...
        # Portrait: yüzdeki "patch" – tek komşuyla neredeyse tam çevrili küçük bölge → birleştir
        if portrait_mode and is_detail_zone and boundary_ratio >= portrait_enclosed_ratio:
    ...
    print(f"     Silinen: {removed_count} (portrait patch: {removed_patch})")
